...
public class LifeTokenItem extends ShinyItem {
    public LifeTokenItem(Item.Settings settings) {
        super(true, Formatting.LIGHT_PURPLE, settings);
    }

    @Override
    public Text getUnformattedName() {
    ...
    public ActionResult use(World world, PlayerEntity user, Hand hand) {
        ...
        if (!PioneersConfig.ENABLE_LIFE_TOKEN) {
            user.sendMessage(Text.translatable("item.pioneers.life_token.use.failure.disabled", getUnformattedName()).formatted(Formatting.RED), false);
            return ActionResult.FAIL;
        }
        Pioneer pioneer = PioneerData.getPioneer(user);
        if (pioneer.getLivesGroup().equals(LivesGroup.GHOST)) {
            user.sendMessage(Text.translatable("item.pioneers.life_token.use.failure.ghost", LivesGroup.GHOST.getDisplayName(), getUnformattedName()).formatted(Formatting.RED), false);
            return ActionResult.FAIL;
        }
        if (pioneer.getLives() == LivesGroup.getTotalLives()) {
            user.sendMessage(Text.translatable("item.pioneers.life_token.use.failure.max_lives", PioneersConfig.getTermForLivesPlural()).formatted(Formatting.RED), false);
            return ActionResult.FAIL;
        }
        pioneer.addLives(1);
        PioneersNetworking.sendToNearbyPlayers((ServerPlayerEntity) user, new PioneerStatusPayload(user, PioneerStatus.GAINED_LIVES));
        user.sendMessage(Text.translatable("lives.gained_lives", Pioneer.getLivesText(1, Formatting.GREEN)), false);
